...
@implementation CRProgressHUD

@synthesize overlayWindow;

+ (CRProgressHUD*)sharedView {
    static dispatch_once_t once;
    static CRProgressHUD *sharedView;
    dispatch_once(&once, ^ {
        sharedView = [[CRProgressHUD alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
...
- (void)show {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!edgeImageView)
            edgeImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:IMGFILE(@"Chat_record_circle")]];
...
{
    [[CRProgressHUD sharedView] setState:str];
}
...
+ (void)dismissWithSuccess:(NSString *)str {
	[[CRProgressHUD sharedView] dismiss:str];
}
...
- (UIWindow *)overlayWindow {
    if(!overlayWindow) {
        overlayWindow = [UIApplication sharedApplication].keyWindow; //[[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
