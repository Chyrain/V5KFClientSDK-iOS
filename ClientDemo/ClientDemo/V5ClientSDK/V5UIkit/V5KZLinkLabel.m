...
- (void)setupTextSystem {
    ...
    _linkDetectionTypes = V5KZLinkDetectionTypeURL|V5KZLinkDetectionTypeHTMLHref|V5KZLinkDetectionTypePhoneNumber;//V5KZLinkDetectionTypeAll;
    ...
    //默认的回调
    self.linkTapHandler = ^(V5KZLinkType linkType, NSString *string) {
        V5Log(@"Link Tap Handler:%@", string);
    };
    self.linkLongPressHandler = ^(V5KZLinkType linkType, NSString *string){
        V5Log(@"Link Long Press Handler:%@", string);
    };
}
...
- (void)setLinkDetectionTypes:(V5KZLinkDetectionTypes)linkDetectionTypes {
...
- (void)updateTextStoreWithAttributedString:(NSAttributedString *)attributedString {
    if (attributedString.length != 0) {
        attributedString = [V5KZLinkLabel sanitizeAttributedString:attributedString];
    }
    ...
    self.V5KZAttributedString = attributedString;
...
//                                 };
    ...
    for (NSDictionary *dictionary in linkRanges) {
        NSRange range = [[dictionary objectForKey:@"range"] rangeValue];
        // 链接地址设置默认链接属性
        if ((V5KZLinkType)[dictionary[@"linkType"] intValue] == V5KZLinkTypeURL ||
            (V5KZLinkType)[dictionary[@"linkType"] intValue] == V5KZLinkTypeHTMLHref) {
...
- (NSArray *)getRangesForLinks:(NSAttributedString **)text {
    ...
    //HTML超链接(将改变内容text)
    if (self.linkDetectionTypes & V5KZLinkDetectionTypeHTMLHref) {
        [rangesForLinks addObjectsFromArray:[self getRangesForHTMLHrefs:text]];
    }
    
    //用户昵称
    if (self.linkDetectionTypes & V5KZLinkDetectionTypeUserHandle) {
        [rangesForLinks addObjectsFromArray:[self getRangesForUserHandles:plainText]];
    }
    //内容标签
    if (self.linkDetectionTypes & V5KZLinkDetectionTypeHashTag) {
        [rangesForLinks addObjectsFromArray:[self getRangesForHashTags:plainText]];
    }
    //链接地址
    if (self.linkDetectionTypes & V5KZLinkDetectionTypeURL) {
        [rangesForLinks addObjectsFromArray:[self getRangesForURLs:*text]];
    }
    //电话号码
    if (self.linkDetectionTypes & V5KZLinkDetectionTypePhoneNumber) {
...
- (NSArray *)getRangesForUserHandles:(NSString *)text {
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        [rangesForUserHandles addObject:@{
                                           @"linkType" : @(V5KZLinkTypeUserHandle),
    ...
    for (NSTextCheckingResult *match in matches) {
        ...
        if ([match resultType] == NSTextCheckingTypePhoneNumber) {
            [rangesForPhoneNumbers addObject:@{
                                                 @"linkType" : @(V5KZLinkTypePhoneNumber),
                                                 @"range"    : [NSValue valueWithRange:matchRange],
                                                 @"link"     : matchString
                                              }];
        }
...
-(BOOL)layoutManager:(NSLayoutManager *)layoutManager shouldBreakLineByWordBeforeCharacterAtIndex:(NSUInteger)charIndex {
    for (NSDictionary *dictionary in self.linkRanges) {
        NSRange range = [[dictionary objectForKey:@"range"] rangeValue];
        V5KZLinkType linkType = [[dictionary objectForKey:@"linkType"] integerValue];
        if (linkType == V5KZLinkTypeURL) {
...
//        NSRange range = [[link objectForKey:@"range"] rangeValue];
        ...
        V5KZLinkType linkType = (V5KZLinkType)[[link objectForKey:@"linkType"] intValue];
        self.linkLongPressHandler(linkType, linkString);
    } else {
        self.linkLongPressHandler(V5KZLinkTypeNone, nil);
    }
}
...
//        NSRange range = [[touchedLink objectForKey:@"range"] rangeValue];
        ...
        V5KZLinkType linkType = (V5KZLinkType)[[touchedLink objectForKey:@"linkType"] intValue];
        
        self.linkTapHandler(linkType, touchedSubstring);
    } else {
        self.linkTapHandler(V5KZLinkTypeNone, nil);
        [super touchesBegan:touches withEvent:event];
    }
    self.selectedRange = NSMakeRange(0, 0);
}
