...
+ (instancetype)resultSetWithStatement:(V5FMStatement *)statement usingParentDatabase:(V5FMDatabase*)aDB {
    ...
    return V5FMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    V5FMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return V5FMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"V5FMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
