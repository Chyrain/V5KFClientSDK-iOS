...
#include "V5AMRFileCodec.h"
int V5AMREncodeMode[] = {4750, 5150, 5900, 6700, 7400, 7950, 10200, 12200}; // amr 编码方式
// 从WAVE文件中跳过WAVE文件头，直接到PCM音频数据
void SkipToPCMAudioData(FILE* fpwave)
{
	V5RIFFHEADER riff;
	V5FMTBLOCK fmt;
	V5XCHUNKHEADER chunk;
	V5WAVEFORMATX wfx;
	int bDataBlock = 0;
	
	// 1. 读RIFF头
	fread(&riff, 1, sizeof(V5RIFFHEADER), fpwave);
	
	// 2. 读FMT块 - 如果 fmt.nFmtSize>16 说明需要还有一个附属大小没有读
	fread(&chunk, 1, sizeof(V5XCHUNKHEADER), fpwave);
	if ( chunk.nChunkSize>16 )
	{
		fread(&wfx, 1, sizeof(V5WAVEFORMATX), fpwave);
	}
	else
	{
		memcpy(fmt.chFmtID, chunk.chChunkID, 4);
		fmt.nFmtSize = chunk.nChunkSize;
		fread(&fmt.wf, 1, sizeof(V5WAVEFORMAT), fpwave);
	}
...
//	unsigned short ush1=0, ush2=0, ush=0;
	...
	// 原始PCM音频帧数据
	unsigned char  pcmFrame_8b1[V5_PCM_FRAME_SIZE];
	unsigned char  pcmFrame_8b2[V5_PCM_FRAME_SIZE<<1];
	unsigned short pcmFrame_16b1[V5_PCM_FRAME_SIZE];
	unsigned short pcmFrame_16b2[V5_PCM_FRAME_SIZE<<1];
	
	if (nBitsPerSample==8 && nChannels==1)
	{

		nRead = fread(pcmFrame_8b1, (nBitsPerSample/8), V5_PCM_FRAME_SIZE*nChannels, fpwave);

		for(x=0; x<V5_PCM_FRAME_SIZE; x++)
		{
		...
		{
			nRead = fread(pcmFrame_8b2, (nBitsPerSample/8), V5_PCM_FRAME_SIZE*nChannels, fpwave);
			for( x=0, y=0; y<V5_PCM_FRAME_SIZE; y++,x+=2 )
			{
				// 1 - 取两个声道之左声道
			...
			{
				nRead = fread(pcmFrame_16b1, (nBitsPerSample/8), V5_PCM_FRAME_SIZE*nChannels, fpwave);
				for(x=0; x<V5_PCM_FRAME_SIZE; x++)
				{
				...
				{
					nRead = fread(pcmFrame_16b2, (nBitsPerSample/8), V5_PCM_FRAME_SIZE*nChannels, fpwave);
					for( x=0, y=0; y<V5_PCM_FRAME_SIZE; y++,x+=2 )
					{
	...
	if (nRead<V5_PCM_FRAME_SIZE*nChannels) return 0;
...
int V5EncodeWAVEFileToAMRFile(const char* pchWAVEFilename, const char* pchAMRFileName, int nChannels, int nBitsPerSample)
{
	FILE* fpwave;
	FILE* fpamr;
	...
	/* bitstream filetype */
	unsigned char amrFrame[V5_MAX_AMR_FRAME_SIZE];
	...
	bytes = fwrite(V5_AMR_MAGIC_NUMBER, sizeof(char), strlen(V5_AMR_MAGIC_NUMBER), fpamr);
...
void WriteWAVEFileHeader(FILE* fpwave, int nFrame)
{
	...
	+ sizeof(V5XCHUNKHEADER)               // fmt
	+ sizeof(V5WAVEFORMATX)           // WAVEFORMATX
	+ sizeof(V5XCHUNKHEADER)               // DATA
	+ nFrame*160*sizeof(short);    //
	strcpy(tag, "WAVE");
	memcpy(riff.chRiffFormat, tag, 4);
	fwrite(&riff, 1, sizeof(V5RIFFHEADER), fpwave);
	...
	chunk.nChunkSize = sizeof(V5WAVEFORMATX);
	fwrite(&chunk, 1, sizeof(V5XCHUNKHEADER), fpwave);
	memset(&wfx, 0, sizeof(V5WAVEFORMATX));
	wfx.nFormatTag = 1;
	wfx.nChannels = 1; // 单声道
	...
	fwrite(&wfx, 1, sizeof(V5WAVEFORMATX), fpwave);
...
int caclAMRFrameSize(unsigned char frameHeader)
{
	...
	mode = V5AMREncodeMode[temp1];
	
	// 计算amr音频数据帧大小
	// 原理: amr 一帧对应20ms，那么一秒有50帧的音频数据
	temp2 = myround((double)(((double)mode / (double)V5_AMR_FRAME_COUNT_PER_SECOND) / (double)8));
...
// 将AMR文件解码成WAVE文件
int V5DecodeAMRFileToWAVEFile(const char* pchAMRFileName, const char* pchWAVEFilename)
{
	...
	unsigned char amrFrame[V5_MAX_AMR_FRAME_SIZE];
	short pcmFrame[V5_PCM_FRAME_SIZE];
...
//	fpamr = fopen([path cStringUsingEncoding:NSASCIIStringEncoding], "rb");
	...
	// 检查amr文件头
	fread(magic, sizeof(char), strlen(V5_AMR_MAGIC_NUMBER), fpamr);
	if (strncmp(magic, V5_AMR_MAGIC_NUMBER, strlen(V5_AMR_MAGIC_NUMBER)))
	{
		fclose(fpamr);
		return 0;
	}
...
//	fpwave = fopen([docFilePath cStringUsingEncoding:NSASCIIStringEncoding], "wb");
	...
	nFrameCount++;
	fwrite(pcmFrame, sizeof(short), V5_PCM_FRAME_SIZE, fpwave);
